...
public class PlayerEvents
{
	...
	public void PlantIntoPot(PlayerInteractEvent event)
	{
		...
		Block interactedBlock = event.getWorld().getBlockState(event.getPos()).getBlock();
		if (heldItem == null || interactedBlock == null) {
			event.setResult(Result.DENY);
			return;
		}
		//Trigger event
		if (event.  == Action.RIGHT_CLICK_BLOCK && heldItem != null && heldItem.getItem() == Item.getItemFromBlock(MoreCraftBlocks.netherwood_sapling)) {
