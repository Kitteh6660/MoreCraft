...
public class NetherwoodBoatEntity extends BoatEntity
{
	   private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(NetherwoodBoatEntity.class, DataSerializers.VARINT);
	   private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(NetherwoodBoatEntity.class, DataSerializers.VARINT);
	   private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(NetherwoodBoatEntity.class, DataSerializers.FLOAT);
	   private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(NetherwoodBoatEntity.class, DataSerializers.VARINT);
	   private static final DataParameter<Boolean> field_199704_e = EntityDataManager.createKey(NetherwoodBoatEntity.class, DataSerializers.BOOLEAN);
	   private static final DataParameter<Boolean> field_199705_f = EntityDataManager.createKey(NetherwoodBoatEntity.class, DataSerializers.BOOLEAN);
	   private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(NetherwoodBoatEntity.class, DataSerializers.VARINT);
	   ...
	   private NetherwoodBoatEntity.Status status;
	   private NetherwoodBoatEntity.Status previousStatus;
	   ...
	   public NetherwoodBoatEntity(EntityType<? extends NetherwoodBoatEntity> p_i50129_1_, World p_i50129_2_) {
	      super(p_i50129_1_, p_i50129_2_);
	      this.preventEntitySpawning = true;
	   }

	   public NetherwoodBoatEntity(World worldIn, double x, double y, double z) {
	   ...
	   protected void registerData() {
	      ...
	      this.dataManager.register(BOAT_TYPE, NetherwoodBoatEntity.Type.NETHERWOOD.ordinal());
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
	   ...
	   public void applyEntityCollision(Entity entityIn) {
	      if (entityIn instanceof NetherwoodBoatEntity || entityIn instanceof BoatEntity) {
	   ...
	   public void tick() {
	      ...
	      if (this.status != NetherwoodBoatEntity.Status.UNDER_WATER && this.status != NetherwoodBoatEntity.Status.UNDER_FLOWING_WATER) {
	    ...
	    */
	   private NetherwoodBoatEntity.Status getBoatStatus() {
		   NetherwoodBoatEntity.Status boatentity$status = this.getUnderwaterStatus();
	      if (boatentity$status != null) {
	         this.waterLevel = this.getBoundingBox().maxY;
	         return boatentity$status;
	      } else if (this.checkInWater()) {
	         return NetherwoodBoatEntity.Status.IN_WATER;
	      } else {
	         float f = this.getBoatGlide();
	         if (f > 0.0F) {
	            this.boatGlide = f;
	            return NetherwoodBoatEntity.Status.ON_LAND;
	         } else {
	            return NetherwoodBoatEntity.Status.IN_AIR;
	         }
	      }
	   }
	         ...
	         for(int k1 = k; k1 < l; ++k1) {
	            ...
	            for(int l1 = i; l1 < j; ++l1) {
	               for(int i2 = i1; i2 < j1; ++i2) {
	                  ...
	                  if (ifluidstate.isTagged(FluidTags.WATER) || ifluidstate.isTagged(FluidTags.LAVA)) {
	    ...
	    */
	   @Nullable
	   private NetherwoodBoatEntity.Status getUnderwaterStatus() {
	      AxisAlignedBB axisalignedbb = this.getBoundingBox();
	         ...
	         for(int k1 = i; k1 < j; ++k1) {
	            for(int l1 = k; l1 < l; ++l1) {
	               for(int i2 = i1; i2 < j1; ++i2) {
	                  blockpos$pooledmutable.setPos(k1, l1, i2);
	                  IFluidState ifluidstate = this.world.getFluidState(blockpos$pooledmutable);
	                  if ((ifluidstate.isTagged(FluidTags.WATER) || ifluidstate.isTagged(FluidTags.LAVA)) && d0 < (double)((float)blockpos$pooledmutable.getY() + ifluidstate.getActualHeight(this.world, blockpos$pooledmutable))) {
	                     if (!ifluidstate.isSource()) {
	                    	 NetherwoodBoatEntity.Status boatentity$status = NetherwoodBoatEntity.Status.UNDER_FLOWING_WATER;
	                        return boatentity$status;
	                     }
	      ...
	      return flag ? NetherwoodBoatEntity.Status.UNDER_WATER : null;
	   }
	   ...
	   private void updateMotion() {
	      ...
	      double d2 = 0.0D;
	      this.momentum = 0.05F;
	      if (this.previousStatus == NetherwoodBoatEntity.Status.IN_AIR && this.status != NetherwoodBoatEntity.Status.IN_AIR && this.status != NetherwoodBoatEntity.Status.ON_LAND) {
	         ...
	         this.lastYd = 0.0D;
	         this.status = NetherwoodBoatEntity.Status.IN_WATER;
	      } else {
	         if (this.status == NetherwoodBoatEntity.Status.IN_WATER) {
	            d2 = (this.waterLevel - this.getPosY()) / (double)this.getHeight();
	            this.momentum = 0.9F;
	         } else if (this.status == NetherwoodBoatEntity.Status.UNDER_FLOWING_WATER) {
	            d1 = -7.0E-4D;
	            this.momentum = 0.9F;
	         } else if (this.status == NetherwoodBoatEntity.Status.UNDER_WATER) {
	            d2 = (double)0.01F;
	            this.momentum = 0.45F;
	         } else if (this.status == NetherwoodBoatEntity.Status.IN_AIR) {
	            this.momentum = 0.9F;
	         } else if (this.status == NetherwoodBoatEntity.Status.ON_LAND) {
	   ...
	   protected void readAdditional(CompoundNBT compound) {
	      if (compound.contains("Type", 8)) {
	         this.setBoatType(NetherwoodBoatEntity.Type.getTypeFromString(compound.getString("Type")));
	      }
	      ...
	      if (!this.isPassenger()) {
	         if (onGroundIn) {
	            if (this.fallDistance > 3.0F) {
	               if (this.status != NetherwoodBoatEntity.Status.ON_LAND) {
	               ...
	               if (!this.world.isRemote && !this.removed) {
	                  ...
	                  if (this.world.getGameRules().getBoolean(GameRules.DO_ENTITY_DROPS)) {
	                     for(int i = 0; i < 3; ++i) {
	                    	 this.entityDropItem(ModItems.NETHERWOOD_PLANKS.get()); //this.entityDropItem(this.getBoatType().asPlank());
	                     }
	            ...
	            this.fallDistance = 0.0F;
	         } else if (!(this.world.getFluidState((new BlockPos(this)).down()).isTagged(FluidTags.WATER) || this.world.getFluidState((new BlockPos(this)).down()).isTagged(FluidTags.LAVA)) && y < 0.0D) {
	   ...
	   public void setBoatType(NetherwoodBoatEntity.Type boatType) {
	      this.dataManager.set(BOAT_TYPE, boatType.ordinal());
	   }

	   /*public NetherwoodBoatEntity.Type getBoatType() {
	      return NetherwoodBoatEntity.Type.byId(this.dataManager.get(BOAT_TYPE));
	   }*/
	   ...
	   public static enum Type {
		  ...
		  //CRIMSON(Blocks.CRIMSON PLANKS, "crimson");
	       ...
	       */
	      public static NetherwoodBoatEntity.Type byId(int id) {
	         NetherwoodBoatEntity.Type[] aboatentity$type = values();
	      ...
	      public static NetherwoodBoatEntity.Type getTypeFromString(String nameIn) {
	    	  NetherwoodBoatEntity.Type[] aboatentity$type = values();
