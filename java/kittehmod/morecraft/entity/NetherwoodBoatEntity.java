...
public class NetherwoodBoatEntity extends BoatEntity
{
          ...
          for(int k1 = i; k1 < j; ++k1) {
             for(int l1 = k; l1 < l; ++l1) {
                for(int i2 = i1; i2 < j1; ++i2) {
                   blockpos$pooledmutableblockpos.setPos(k1, l1, i2);
                   IFluidState ifluidstate = this.world.getFluidState(blockpos$pooledmutableblockpos);
                   if ((ifluidstate.isTagged(FluidTags.WATER) || ifluidstate.isTagged(FluidTags.LAVA)) && d0 < (double)((float)blockpos$pooledmutableblockpos.getY() + ifluidstate.func_215679_a(this.world, blockpos$pooledmutableblockpos))) {
             ...
             if (this.fallDistance > 3.0F) {
                ...
                if (!this.world.isRemote && !this.removed) {
                   ...
                   if (this.world.getGameRules().getBoolean(GameRules.DO_ENTITY_DROPS)) {
                      for(int i = 0; i < 3; ++i) {
                         this.entityDropItem(ModItems.NETHERWOOD_PLANKS.get());
                      }
    ...
    protected void givePassengersImmunity() {
    	for (int i = 0; i < this.getPassengers().size(); i++) {
    		if (this.getPassengers().get(i) != null && this.getPassengers().get(i) instanceof LivingEntity) {
	        	LivingEntity living = (LivingEntity)this.getPassengers().get(i);
	        	if (!living.isPotionActive(Effects.FIRE_RESISTANCE)) {
	        		living.addPotionEffect(new EffectInstance(Effects.FIRE_RESISTANCE, 20, 0, true, false));
	        	}
    		}
    	}
    	/*LivingEntity living = (LivingEntity)passenger;
    	if (!living.isPotionActive(Effects.FIRE_RESISTANCE)) {
    		living.addPotionEffect(new EffectInstance(Effects.FIRE_RESISTANCE, 10, 0, false, false));
    	}*/
    }
