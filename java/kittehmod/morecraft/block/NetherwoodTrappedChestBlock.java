...
public class NetherwoodTrappedChestBlock extends AbstractChestBlock<NetherwoodTrappedChestTileEntity> implements IWaterLoggable
{
	...
	private static final TileEntityMerger.ICallback<NetherwoodTrappedChestTileEntity, Optional<IInventory>> field_220109_i = new TileEntityMerger.ICallback<NetherwoodTrappedChestTileEntity, Optional<IInventory>>() {
		public Optional<IInventory> func_225539_a_(NetherwoodTrappedChestTileEntity p_225539_1_, NetherwoodTrappedChestTileEntity p_225539_2_) {
			return Optional.of(new DoubleSidedInventory(p_225539_1_, p_225539_2_));
		}

		public Optional<IInventory> func_225538_a_(NetherwoodTrappedChestTileEntity p_225538_1_) {
	...
	private static final TileEntityMerger.ICallback<NetherwoodTrappedChestTileEntity, Optional<INamedContainerProvider>> field_220110_j = new TileEntityMerger.ICallback<NetherwoodTrappedChestTileEntity, Optional<INamedContainerProvider>>() {
		public Optional<INamedContainerProvider> func_225539_a_(final NetherwoodTrappedChestTileEntity p_225539_1_, final NetherwoodTrappedChestTileEntity p_225539_2_) {
		...
		public Optional<INamedContainerProvider> func_225538_a_(NetherwoodTrappedChestTileEntity p_225538_1_) {
			return Optional.of(p_225538_1_);
		}
	...
	/*
	 * protected NetherwoodTrappedChestBlock(Block.Properties builder,
	 * Supplier<TileEntityType<? extends NetherwoodTrappedChestTileEntity>>
	 * tileEntityTypeIn) { super(builder, tileEntityTypeIn);
	 * this.setDefaultState(this.stateContainer.getBaseState().with(FACING,
	 * Direction.NORTH).with(TYPE, ChestType.SINGLE).with(WATERLOGGED,
	 * Boolean.valueOf(false))); }
	 */

	protected NetherwoodTrappedChestBlock(Block.Properties builder, Supplier<TileEntityType<? extends NetherwoodTrappedChestTileEntity>> tileEntityTypeIn) {
	...
	@Nullable
	public static IInventory func_226916_a_(NetherwoodTrappedChestBlock chestBlockIn, BlockState blockStateIn, World worldIn, BlockPos blockPosIn, boolean isDouble) {
		return chestBlockIn.combine(blockStateIn, worldIn, blockPosIn, isDouble).apply(field_220109_i).orElse((IInventory) null);
	}

	@Override
	public TileEntityMerger.ICallbackWrapper<? extends NetherwoodTrappedChestTileEntity> combine(BlockState blockStateIn, World worldIn, BlockPos blockPosIn, boolean isDouble) {
		BiPredicate<IWorld, BlockPos> bipredicate;
		if (isDouble) {
			bipredicate = (p_226918_0_, p_226918_1_) -> { return false; };
		} else {
			bipredicate = NetherwoodTrappedChestBlock::isBlocked;
		}
		return TileEntityMerger.func_226924_a_(this.tileEntityType.get(), NetherwoodTrappedChestBlock::func_226919_h_, NetherwoodTrappedChestBlock::getDirectionToAttached, FACING, blockStateIn, worldIn, blockPosIn, bipredicate);
	}

	@Nullable
	public INamedContainerProvider getContainer(BlockState state, World worldIn, BlockPos pos) {
		return this.combine(state, worldIn, pos, false).apply(field_220110_j).orElse((INamedContainerProvider) null);
	}

	@OnlyIn(Dist.CLIENT)
	public static TileEntityMerger.ICallback<NetherwoodTrappedChestTileEntity, Float2FloatFunction> func_226917_a_(final IChestLid chestLidIn) {
		return new TileEntityMerger.ICallback<NetherwoodTrappedChestTileEntity, Float2FloatFunction>() {
			public Float2FloatFunction func_225539_a_(NetherwoodTrappedChestTileEntity tileEntityIn1, NetherwoodTrappedChestTileEntity tileEntityIn2) {
				return (chestLid) -> { return Math.max(tileEntityIn1.getLidAngle(chestLid), tileEntityIn2.getLidAngle(chestLid)); };
			}

			public Float2FloatFunction func_225538_a_(NetherwoodTrappedChestTileEntity tileEntityIn) {
				return tileEntityIn::getLidAngle;
			}

			public Float2FloatFunction func_225537_b_() {
				return chestLidIn::getLidAngle;
			}
		};
	}
	...
	public static boolean isBlocked(IWorld worldIn, BlockPos posIn) {
		return isBelowSolidBlock(worldIn, posIn) || isCatSittingOn(worldIn, posIn);
	}

	private static boolean isBelowSolidBlock(IBlockReader readerIn, BlockPos worldIn) {
		BlockPos blockpos = worldIn.up();
		return readerIn.getBlockState(blockpos).isNormalCube(readerIn, blockpos);
	}

	private static boolean isCatSittingOn(IWorld worldIn, BlockPos posIn) {
		List<CatEntity> list = worldIn
				.getEntitiesWithinAABB(CatEntity.class, new AxisAlignedBB((double) posIn.getX(), (double) (posIn.getY() + 1), (double) posIn.getZ(), (double) (posIn.getX() + 1), (double) (posIn.getY() + 2), (double) (posIn.getZ() + 1)));
		if (!list.isEmpty()) {
