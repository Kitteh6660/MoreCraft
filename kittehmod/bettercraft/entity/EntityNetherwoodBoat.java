package kittehmod.bettercraft.entity;
...
{
    private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityNetherwoodBoat.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityNetherwoodBoat.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityNetherwoodBoat.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityNetherwoodBoat.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean>[] DATA_ID_PADDLE = new DataParameter[] {EntityDataManager.createKey(EntityNetherwoodBoat.class, DataSerializers.BOOLEAN), EntityDataManager.createKey(EntityNetherwoodBoat.class, DataSerializers.BOOLEAN)};
    ...
    private EntityNetherwoodBoat.Status status;
    private EntityNetherwoodBoat.Status previousStatus;
    private double lastYd;

    public EntityNetherwoodBoat(World worldIn)
    {
        super(worldIn);
    ...
    public EntityNetherwoodBoat(World worldIn, double x, double y, double z)
    {
        this(worldIn);
    ...
    {
        if (entityIn instanceof EntityNetherwoodBoat)
        {
    ...
    {
    	return MoreCraftItems.NETHERWOOD_BOAT;
    }
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityNetherwoodBoat.Status.UNDER_WATER && this.status != EntityNetherwoodBoat.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityNetherwoodBoat.Status getBoatStatus()
    {
        EntityNetherwoodBoat.Status EntityNetherwoodBoat$status = this.getUnderLiquidStatus();

        if (EntityNetherwoodBoat$status != null)
        {
            this.waterLevel = this.getEntityBoundingBox().maxY;
            return EntityNetherwoodBoat$status;
        }
        else if (this.checkInLiquid())
        {
            return EntityNetherwoodBoat.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityNetherwoodBoat.Status.ON_LAND;
            }
            else
            {
                return EntityNetherwoodBoat.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityNetherwoodBoat.Status getUnderLiquidStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if ((iblockstate.getMaterial() == Material.WATER || iblockstate.getMaterial() == Material.LAVA) && d0 < (double)getLiquidHeight(iblockstate, this.world, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityNetherwoodBoat.Status EntityNetherwoodBoat$status = EntityNetherwoodBoat.Status.UNDER_FLOWING_WATER;
                                return EntityNetherwoodBoat$status;
                            }
        ...
        return flag ? EntityNetherwoodBoat.Status.UNDER_WATER : null;
    }
    ...
    public static float getBlockLiquidHeight(IBlockState p_184456_0_, IBlockAccess p_184456_1_, BlockPos p_184456_2_)
    {
        ...
        return (i & 7) == 0 && (p_184456_1_.getBlockState(p_184456_2_.up()).getMaterial() == Material.WATER || p_184456_1_.getBlockState(p_184456_2_.up()).getMaterial() == Material.LAVA) ? 1.0F : 1.0F - BlockLiquid.getLiquidHeightPercent(i);
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityNetherwoodBoat.Status.IN_AIR && this.status != EntityNetherwoodBoat.Status.IN_AIR && this.status != EntityNetherwoodBoat.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getLiquidLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityNetherwoodBoat.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityNetherwoodBoat.Status.IN_WATER)
            {
            ...
            else if (this.status == EntityNetherwoodBoat.Status.UNDER_FLOWING_WATER)
            {
                d1 = -7.0E-4D;
                this.momentum = 0.9F;
                if (this.isInLava()) {
            ...
            else if (this.status == EntityNetherwoodBoat.Status.ON_LAND)
            {
    ...
    protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos)
    {
        ...
        if (!this.isRiding())
        {
            if (onGroundIn)
            {
                ...
                {
                    if (this.status != EntityNetherwoodBoat.Status.ON_LAND)
                    {
                    ...
                    if (!this.world.isRemote && !this.isDead)
                    {
                        ...
                        if (this.world.getGameRules().getBoolean("doEntityDrops"))
                        {
                            for (int i = 0; i < 3; ++i)
                            {
                                this.entityDropItem(new ItemStack(Item.getItemFromBlock(MoreCraftBlocks.NETHERWOOD_PLANKS), 1, 0), 0.0F);
                            }
