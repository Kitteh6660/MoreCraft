...
{
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(NetherBoatEntity.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(NetherBoatEntity.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(NetherBoatEntity.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(NetherBoatEntity.class, DataSerializers.VARINT);
	private static final DataParameter<Boolean> field_199704_e = EntityDataManager.createKey(NetherBoatEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> field_199705_f = EntityDataManager.createKey(NetherBoatEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(NetherBoatEntity.class, DataSerializers.VARINT);
	...
	private NetherBoatEntity.Status previousStatus;
	...
	protected void registerData() {
		...
		this.dataManager.register(BOAT_TYPE, NetherBoatEntity.Type.NETHERWOOD.ordinal());
	 ...
	 */
	private NetherBoatEntity.Status getBoatStatus() {
		NetherBoatEntity.Status boatentity$status = this.getUnderwaterStatus();
	 ...
	 */
	@Nullable
	private NetherBoatEntity.Status getUnderwaterStatus() {
		AxisAlignedBB axisalignedbb = this.getBoundingBox();
	...
	private void updateMotion() {
		...
		double d2 = 0.0D;
		this.momentum = 0.05F;
		if (this.previousStatus == NetherBoatEntity.Status.IN_AIR && this.status != NetherBoatEntity.Status.IN_AIR && this.status != NetherBoatEntity.Status.ON_LAND) {
	...
	protected void readAdditional(CompoundNBT compound) {
		if (compound.contains("Type", 8)) {
			this.setBoatType(NetherBoatEntity.Type.getTypeFromString(compound.getString("Type")));
		}
				...
				if (this.fallDistance > 3.0F) {
					...
					if (!this.world.isRemote && !this.removed) {
						...
						if (this.world.getGameRules().getBoolean(GameRules.DO_ENTITY_DROPS)) {
							for (int i = 0; i < 3; ++i) {
								this.entityDropItem(this.getNetherBoatType().asPlank());
							}
	...
	public NetherBoatEntity.Type getNetherBoatType() {
		return NetherBoatEntity.Type.byId(this.dataManager.get(BOAT_TYPE));
	}
	...
	public static enum Type
	{
		...
		public static NetherBoatEntity.Type getTypeFromString(String nameIn) {
			NetherBoatEntity.Type[] aboatentity$type = values();
